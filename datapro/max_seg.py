...
class PsegMax:
    ...
    def load_entity(self, dict_path):
        ...
        df = pd.read_csv(dict_path,header=None,names=["entity","tag"],encoding="gbk")
        entity_dict = {entity.strip():self.chinese2englishlabel[tag.strip()] for entity,tag in df.values.tolist()}
